...
instance (Elt a, Elt b) => IsTuple (a, b)

-- type TupleR (a, b) = (((), a), b)
-- fromTup (a, b)     = (((), a), b)
-- toTup (((), a), b) = (a, b)

instance (Elt a, Elt b, Elt c) => IsTuple (a, b, c)
...
instance (GIsSumTuple a, GIsSumTuple b) => GIsSumTuple (a :+: b) where
  ...
  gsumFromTup n t (L1 a) =
    let (m, r) = gsumFromTup n t a
     in (shiftL m 1, gsumUndef @b r)
  gsumFromTup n t (R1 b) =
    let (m, r) = gsumFromTup n (gsumUndef @a t) b
